...
def get_pfam_AF(conf):
    ...
    for row in mycursor.fetchall():
        try:
            list_pfam_AF[row[0]][row[1]] = [row[2],row[3]]
        except KeyError:
            list_pfam_AF[row[0]] = {row[1]:[row[2],row[3]]}
    ...
    with open (af2pfam_file, "w") as af2pf:
        ...
        for pfam, af_dict in list_pfam_af.items():
            ...
            for af, match in af_dict.items():
                ...
                count+=1
                af2pf.write(f"{af},{pfam},{pf_dict[pfam]['id']},{pf_dict[pfam]['clan']},{pf_dict[pfam]['desc']},{pf_dict[pfam]['struct']},{match[0]},{match[1]}\n")

    print(len(af2process))
    return af2process

# def write_in_file(list_pfam_af, pf_dict, pf_struct):
#     af2process=dict()
#     with open (af2pfam_file, "w") as af2pf:
#         af2pf.write("af_prot,pfam_acc,pfam_id,clan,pfam_desc,has_struct,af_start,af_end\n")
#         for pfam in pf_dict.keys():
#             count = 0
#             if pfam in pf_struct:
#                 for af, match in list_pfam_af[pfam].items():
#                     if count == 0: #only get file for first AF encountered
#                         get_af_file(outputdir, af)
#                     count+=1
#                     af2pf.write(f"{af},{pfam},{pf_dict[pfam]['id']},{pf_dict[pfam]['clan']},{pf_dict[pfam]['desc']},{match[0]},{match[1]}\n")
#                     try:
#                         af2process[af].append(pfam)
#                     except KeyError:
#                         af2process[af]=[pfam]
#     print(len(af2process))
#     return af2process
